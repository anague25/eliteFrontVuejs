<template>
    ...
    <div class="table-container card">
        <div class="card-body">
            <h3 class="text-center text-uppercase text-white fw-bold mb-3">liste des spécialités</h3>
            <div class="card-title my-0 py-0 ">
                <router-link to="/admin-create-specialty" class="float-end btn btn-sm btn-success ">Créer des
                    spécialités</router-link>
                <span class="row col-md-6 col-4 "><input type="search" v-model="searchQuery" @input="searchSpecialties"
            ...
            <table class="custom-table">
                ...
                <tbody>
                    <tr v-for="(specialty, index) in specialtyStore.specialties" :key="specialty.id">
                        <td>{{ index + 1 }}</td>
                        <td>
                            <input type="checkbox" :value="specialty.id" v-model="selectedSpecialties">
                        </td>
                        <td>{{ specialty.name }}</td>
                        <td>{{ specialty.instructor?.user?.firstName }}</td>
                    ...
                    <tr v-if="specialtyStore.specialties.length === 0">
                        <td colspan="3" class="text-center">Aucune spécialité n'a été trouvé.</td>
                    </tr>
...
const toggleSelectAll = () => {
    if (selectAll.value) {
        selectedSpecialties.value = specialtyStore.specialties.map(specialty => specialty.id);
...
const editSpecialty = (id) => {
    router.push({ name: "editSpecialties", params: { id } });
};
...
const searchSpecialties = () => {
    specialtyStore.fetchSpecialties(page.value, searchQuery.value);
};


const fetchPage = (page) => {
    if (page) {
        specialtyStore.fetchSpecialties(page, searchQuery.value);
    }
};
