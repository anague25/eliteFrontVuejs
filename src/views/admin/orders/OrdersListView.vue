<template>
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active text-success">liste des ventes</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="table-container card">
        <div class="card-body">
            <h3 class="text-center text-uppercase text-white fw-bold mb-3">listes des Ventes</h3>
            <div class="card-title my-0 py-0 ">

                <span class="row col-md-6 col-4 "><input type="search" v-model="searchQuery" @input="searchorders"
                        placeholder="Rechercher" class="form-control mb-2"></span>
            </div>
            <div class="d-flex flex-wrap gap-2 ">
                <div class="card" v-for="(order, index) in courseStore.orders" :key="order.id" style="width: 30rem;">
                    <div class="card-header">
                        <span style="font-size: 15px;">Num de commande : {{ order.number }}</span> <br>
                        <span>Montant :<em class="text-success"> {{ (Number(order.amount)).toFixed(2) }} XAF
                            </em></span>
                    </div>
                    <div class="card-body d-flex flex-wrap gap-1 justify-content-center">
                        <router-link v-for="course in order.orderCourses" :to="'/home-course/' + course.id" class="card"
                            style="width: 11rem;">
                            ...
                            <ul class="list-group list-group-flush p-0 m-0">
                                ...
                                <li class="list-group-item p-0 m-1 " style="font-size: 14px;">Prix : {{ course.price
                                }}</li>
...
const searchorders = (pag) => {
    courseStore.adminOrder(pag, searchQuery.value);
};


const fetchPage = (page) => {
    if (page) {
        courseStore.adminOrder(page, searchQuery.value);
    }
};
