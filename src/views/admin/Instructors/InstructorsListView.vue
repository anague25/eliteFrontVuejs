<template>
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active text-success">liste des instructeurs</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="table-container card">
        <div class="card-body">
            <h3 class="text-center text-uppercase text-white fw-bold mb-3">listes des instructeurs</h3>
            <div class="card-title my-0 py-0 ">
                <router-link to="/admin-create-instructor" class="float-end btn btn-sm btn-success ">Créer un
                    instructeur</router-link>
            ...
            <table class="custom-table">
                ...
                <tbody>
                    <tr v-for="(instructor, index) in instructorStore.instructors" :key="instructor.id">
                        <td>{{ index + 1 }}</td>
                        <td>
                            <input type="checkbox" :value="instructor.id" v-model="selectedInstructors">
                        </td>
                        ...
                        <td class=""> <img class="img" :src="$baseUrl + instructor.image" alt="" width="28" height="28" />
                        </td>
                        <td class=""> <img class="img" :src="$baseUrl + instructor.idCart1" alt="" width="28" height="28" />
                        </td>
                        <td class=""> <img class="img" :src="$baseUrl + instructor.idCart2" alt="" width="28" height="28" />
                        </td>
                    ...
                    <tr v-if="instructorStore.instructors.length === 0">
                        <td colspan="3" class="text-center">Aucun instructeur n'a été trouvé.</td>
                    </tr>

                </tbody>
            </table>
            <!-- Bouton pour suppression multiple -->
            <button class="btn btn-danger mt-2 float-end" @click="deleteMultipleInstructors"
        ...
        <div class="container-fluid ">
            <div class="pagination d-flex gap-2 float-end flex-wrap">
                <!-- Bouton "Précédent" -->
                <button class="btn btn-dark btn-pag" :disabled="instructorStore.pagination.current_page === 1"
                    @click="fetchPage(instructorStore.pagination.current_page - 1)">
                    &laquo; Précédent
                </button>
                ...
                <button class="btn btn-dark btn-pag"
                    :disabled="instructorStore.pagination.current_page === instructorStore.pagination.last_page"
                    @click="fetchPage(instructorStore.pagination.current_page + 1)">
                    Suivant &raquo;
                </button>
            </div>
...
const toggleSelectAll = () => {
    if (selectAll.value) {
        selectedInstructors.value = instructorStore.instructors.map(instructeur => instructeur.id);
        console.log(selectedInstructors.value);
    } else {
...
const deleteInstructor = async (id) => {
    confirmModal.value.show(
        "Voulez-vous vraiment supprimer cet instructeur ?",
        async () => {
...
const editInstructor = (id) => {
    router.push({ name: "editInstructors", params: { id } });
};




const deleteMultipleInstructors = async () => {
    confirmModal.value.show(
        "Voulez-vous vraiment supprimer les Instructeur sélectionnés ?",
        async () => {
            await instructorStore.deleteMultipleInstructors(selectedInstructors.value);
            selectedInstructors.value = [];
...
const searchInstructors = () => {
    instructorStore.fetchInstructors(page.value, searchQuery.value);
};


const fetchPage = (page) => {
    if (page) {
        instructorStore.fetchInstructors(page, searchQuery.value);
    }
};

// generer des pages intermediaires
const formattedLinks = computed(() => {
    const { current_page, last_page } = instructorStore.pagination;
    const links = [];
