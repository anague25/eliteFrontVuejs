<template>
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active text-success">liste des secteurs</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="table-container card">
        <div class="card-body">
            <h3 class="text-center text-uppercase text-white fw-bold mb-3">liste des secteurs</h3>
            <div class="card-title my-0 py-0 ">
                <router-link to="/admin-create-sector" class="float-end btn btn-sm btn-success ">Créer des
                    secteurs</router-link>
                <span class="row col-md-6 col-4 "><input type="search" v-model="searchQuery" @input="searchSectors"
            ...
            <table class="custom-table">
                ...
                <tbody>
                    ...
                    <tr v-for="(sector, index) in sectorStore.sectors" :key="sector.id">
                        <td>{{ index + 1 }}</td>
                        <td>
                            <input type="checkbox" :value="sector.id" v-model="selectedSectors">
                        </td>
                        <td>{{ sector.name }}</td>
                        <td class="d-flex">
                            <button class="btn btn-sm btn-danger ms-1"
                                @click="deleteSector(sector.id)">Supprimer</button>
                            <button class="btn btn-sm btn-primary ms-1" @click="editSector(sector.id)">Modifier</button>

                        </td>
                    </tr>
                    <tr v-if="sectorStore.sectors.length === 0">
                        <td colspan="3" class="text-center">Aucun secteur n'a été trouvé.</td>
                    </tr>

                </tbody>
            </table>
            <!-- Bouton pour suppression multiple -->
            <button class="btn btn-danger mt-2 float-end" @click="deleteSelectedSectors"
        ...
        <div class="container-fluid ">
            <div class="pagination d-flex gap-2 float-end flex-wrap">
                <!-- Bouton "Précédent" -->
                <button class="btn btn-dark btn-pag" :disabled="sectorStore.pagination.current_page === 1"
                    @click="fetchPage(sectorStore.pagination.current_page - 1)">
                    &laquo; Précédent
                </button>
                ...
                <button class="btn btn-dark btn-pag"
                    :disabled="sectorStore.pagination.current_page === sectorStore.pagination.last_page"
                    @click="fetchPage(sectorStore.pagination.current_page + 1)">
                    Suivant &raquo;
                </button>
            </div>
...
const toggleSelectAll = () => {
    if (selectAll.value) {
        selectedSectors.value = sectorStore.sectors.map(sector => sector.id);
    } else {
...
const deleteSector = async (id) => {
    confirmModal.value.show(
        "Voulez-vous vraiment supprimer ce secteur ?",
        async () => {
            ...
            sectorStore.fetchSectors(); // Rafraîchir la liste
        }
    );
};
...
const editSector = (id) => {
    router.push({ name: "editSectors", params: { id } });
};

// Fonction pour supprimer les secteurs sélectionnés
const deleteSelectedSectors = async () => {
    confirmModal.value.show(
        "Voulez-vous vraiment supprimer les secteurs sélectionnés ?",
        async () => {
            await sectorStore.deleteMultipleSectors(selectedSectors.value);
            selectedSectors.value = [];
...
const searchSectors = () => {
    sectorStore.fetchSectors(page.value, searchQuery.value);
};


const fetchPage = (page) => {
    if (page) {
        sectorStore.fetchSectors(page, searchQuery.value);
    }
};

// generer des pages intermediaires
const formattedLinks = computed(() => {
    const { current_page, last_page } = sectorStore.pagination;
    const links = [];
