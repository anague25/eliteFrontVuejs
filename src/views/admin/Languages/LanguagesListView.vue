<template>
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active text-success">liste des langues</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="table-container card">
        <div class="card-body">
            <h3 class="text-center text-uppercase text-white fw-bold mb-3">listes des langues</h3>
            <div class="card-title my-0 py-0 ">
                <router-link to="/admin-create-language" class="float-end btn btn-sm btn-success ">Créer des
                    langues</router-link>
                <span class="row col-md-6 col-4 "><input type="search" v-model="searchQuery" @input="searchLanguages"
            ...
            <table class="custom-table">
                ...
                <tbody>
                    <tr v-for="(language, index) in languageStore.languages" :key="language.id">
                        <td>{{ index + 1 }}</td>
                        <td>
                            <input type="checkbox" :value="language.id" v-model="selectedLanguages">
                        </td>
                        <td>{{ language.name }}</td>

                        <td class="d-flex ">
                            <button class="btn btn-sm btn-danger my-1 ms-1"
                                @click="deleteLanguage(language.id)">Supprimer</button>
                            <button class="btn btn-sm btn-primary my-1 ms-1"
                                @click="editLanguage(language.id)">Modifier</button>

                        </td>
                    </tr>
                    <tr v-if="languageStore.languages.length === 0">
            ...
            <button class="btn btn-danger mt-2 float-end" @click="deleteMultipleLanguages"
        ...
        <div class="container-fluid ">
            <div class="pagination d-flex gap-2 float-end flex-wrap">
                <!-- Bouton "Précédent" -->
                <button class="btn btn-dark btn-pag" :disabled="languageStore.pagination.current_page === 1"
                    @click="fetchPage(languageStore.pagination.current_page - 1)">
                    &laquo; Précédent
                </button>
                ...
                <button class="btn btn-dark btn-pag"
                    :disabled="languageStore.pagination.current_page === languageStore.pagination.last_page"
                    @click="fetchPage(languageStore.pagination.current_page + 1)">
                    Suivant &raquo;
                </button>
            </div>
...
const toggleSelectAll = () => {
    if (selectAll.value) {
        selectedLanguages.value = languageStore.languages.map(language => language.id);
...
const editLanguage = (id) => {
    router.push({ name: "editlanguages", params: { id } });
};




const deleteMultipleLanguages = async () => {
    confirmModal.value.show(
        "Voulez-vous vraiment supprimer les Langues sélectionnés ?",
        async () => {
            await languageStore.deleteMultipleLanguages(selectedLanguages.value);
            selectedLanguages.value = [];
...
const searchLanguages = () => {
    languageStore.fetchLanguages(page.value, searchQuery.value);
};


const fetchPage = (page) => {
    if (page) {
        languageStore.fetchLanguages(page, searchQuery.value);
    }
};

// generer des pages intermediaires
const formattedLinks = computed(() => {
    const { current_page, last_page } = languageStore.pagination;
    const links = [];
