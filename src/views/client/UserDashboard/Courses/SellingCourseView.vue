<template>
    <main id="main" class="main">
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active text-success">liste de vos ventes</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="table-container card">
            <div class="card-body">
                <h3 class="text-center text-uppercase text-white fw-bold mb-3">listes de vos ventes</h3>
                ...
                <div class="d-flex flex-wrap gap-2 ">
                    <div class="card" v-for="(order, index) in courseStore.orders" :key="order.id"
                        >
                        <div class="card-header">
                            <span style="font-size: 15px;">Num de commande : {{ order.number }}</span>
                        </div>
                        <div class="card-body d-flex flex-wrap gap-1 justify-content-center">
                            <router-link  v-for="course in order.orderCourses" :to="'/home-course/' + course.id" class="card" style="width: 13rem;">
                ...
                <div v-if="courseStore.orders.length === 0">
                    <p colspan="3" class="text-center">Aucune vente</p>
                </div>
...
const searchCourses = () => {
    courseStore.soldCourses(page.value, searchQuery.value);
};


const fetchPage = (page) => {
    if (page) {
        courseStore.soldCourses(page, searchQuery.value);
    }
};
