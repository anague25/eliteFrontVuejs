    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active text-success">édition d'une ressource</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="bgcolor2">
        ...
        <section class="form-section">
            ...
            <div class="form-container bgcolor1">
                <h2 class="text-center text-white fw-bold">Modifier une ressource</h2>
                <form @submit.prevent="updateResource" class="text">
                    <div class="mb-3">
                        <label class="form-label">Cours</label>
                        <select v-model="selectedCourse" class="form-control">
                            <option value="" disabled selected>Veuillez choisir une resource</option>
                            <option v-for="course in courseStore.getCoursesInstructor" :key="course.id" :value="course.id">
                                {{ course.title }}
                            </option>
                        </select>
                    ...
                    <div v-else class="mt-1 mb-2">
                        <img class="img" :src="$baseUrl + resource.file" alt="" width="60" height="60" />
                    </div> -->
                ...
                <div class="mt-2"><router-link to="/users-dashboard/resources" class="text-decoration-none">Retour</router-link>
                </div>
            </div>
        </section>
    </div>
...
onMounted(async () => {
    ...
    try {
        ...
        data.value = await resourceStore.fetchResourceById(route.params.id);
        resource.value.id = data.value.id;
        resource.value.chapter_id = data.value.chapter_id;
...
const updateResource = async () => {
    try {
        ...
        if (!(resource.value.file instanceof File)) {
            delete resource.value.file;
        }
        ...
        router.push("/users-dashboard/resources");
    }
    catch (error) {
