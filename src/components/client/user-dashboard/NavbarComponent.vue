<template>
  <nav class="header-nav ms-auto ">
    <ul class="d-flex align-items-center">
      <li class="me-3"><router-link class="nav-item  white" aria-current="page" to="/">Accueil</router-link></li>
      ...
      <li class="nav-item dropdown ">
        ...
        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow notifications box-notification">
          <li class="dropdown-header text-white">
            ...
            <router-link to="/users-dashboard/notifications">
          ...
          <li class="dropdown-footer">
            <router-link to="/users-dashboard/notifications" class="text-white">Voir toutes les
      ...
      <li class="nav-item dropdown pe-3">
        ...
        <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
          <img v-if="authStore.getuser.user && authStore.getuser?.user.firstName"
            :src="$baseUrl + authStore.getuser.user?.profile" alt="Profile" class="rounded-circle">
          <span v-if="authStore.getuser.user && authStore.getuser?.user.firstName"
            class="d-none d-md-block dropdown-toggle ps-2 text-white">{{ authStore.getuser.user?.firstName?.slice(0, 1)
            }}.
        ...
        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile box-profile" id="profile">
          <li class="dropdown-header">
            <h6 v-if="authStore.getuser.user && authStore.getuser?.user.firstName" class="text-white">{{
              authStore.getuser.user?.firstName }}</h6>
            <!-- <span class="text-white">Web Designer</span> -->
            <span v-if="authStore.getuser.user && authStore.getuser?.user.firstName" class="text-white">{{
              authStore.getuser.user?.lastName }}</span>
          </li>
          <li>
            <hr class="dropdown-divider">
